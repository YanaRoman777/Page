$(document).ready(function(){
 ...
 function topFunction() {
    ...
    window.addEventListener('resize', function () {
        ...
        const tokenInfoTable = document.querySelector('.token-info__table');
        const tokenInfoText = document.querySelector('.token-info__text_mobail');
        const pen = document.querySelector('.pen-button_mobail');
        const tokenInfoMobail = document.querySelector('.token-info_mobail');

        if (window.innerWidth <= 930) {
          // 0...930
          tokenInfoMobail.prepend(tokenInfoText);
          tokenInfoMobail.append(pen);
        } else {
          // 930...Inf
          tokenInfoTable.after(tokenInfoText);
          tokenInfo.append(pen);
        }
      });
